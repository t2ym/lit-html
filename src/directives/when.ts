...
export const when =
    (condition: any, trueValue: () => any, falseValue: () => any) =>
        new WhenDirective(condition, trueValue, falseValue);
...
class WhenDirective extends Directive {
  ...
  constructor(condition: any, trueValue: () => any, falseValue: () => any) {
    super(when, [condition, trueValue, falseValue]);
    this.condition = condition;
    this.trueValue = trueValue;
    this.falseValue = falseValue;
  }

  commit(parentPart: NodePart) {
    ...
    // to set on that part.
    const nextPart = this.condition ? cache.truePart : cache.falsePart;
    const nextValue = this.condition ? this.trueValue() : this.falseValue();

    // If we switched condition, swap nodes to/from the cache.
    if (!!this.condition !== cache.prevCondition) {
      // Get the part which was rendered for the opposite condition. This
      // should be added to the cache.
      const prevPart = this.condition ? cache.falsePart : cache.truePart;
    ...
    cache.prevCondition = !!this.condition;
  }
}
